<template>
  <div>
    <div class="layout-admin-background" id="layout-admin-background"></div>
    <div class="mask-layer">
      <div class="info-content">
        <div class="avatar">
          <transition name="loader-avatar">
            <img v-if="avatarImagePath" :src="require('@/assets/images/' + avatarImagePath)">
...
const createElements = function() {
  const elements = [];
  for (let x = 20; x < EChart.getWidth(); x += 40) {
    for (let y = 20; y < EChart.getHeight(); y += 40) {
...
export default {
  ...
  computed: {
    ...mapState('login', ['avatarImagePath'])
  },
  mounted() {
    this.EChartsInit()
  ...
  methods: {
    ...
    submitAdmin() {
      ...
      if (!username) {
        ...
        this.usernameErrorMsg = '请输入登录名'
      }
      if (!password) {
        this.passwordError = true
        this.passwordErrorMsg = '请输入登录密码'
      }
      if (username && password) {
        this.storeSubmitAdmin({ username, password }).then(res => {
          if (res) {
    ...
    EChartsInit() {
      let dom = document.getElementById('layout-admin-background')
