<template>
  <div class="layout-header">
    ...
    <div :class="{ 'icon-item': true, 'search-selected': searchMenuFlag }" @click.stop="searchMenu">
      <i class="iconfont icon-search"></i>
      <input type="text" class="search-menu-input" ref="search-menu-input" v-model="searchMenuKeywords" @input="searchMenuInput">
    </div>
    <div :class="{ 'menu-list': true, 'menu-list-selected': searchMenuFlag }" @click.stop>
      <div v-for="item in searchMenuList" :key="item.path" @click="pathTo(item.path)">{{ item.label }}</div>
    </div>
  </div>
</template>
<script>
